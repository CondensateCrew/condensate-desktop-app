...
const RoundTwo: React.FC = () => {
  ...
  const prompts: WordSample[] = exampleSentences.filter((wordObj: WordSample) => chosenWords.includes(wordObj.word))
  ...
  const insights: JSX.Element[] = responses.map((insight: string, idx: number) => (
    <GenerateInsights
      text={insight}
      responses={responses}
      key={idx}
